...
enum AppGroupShared {
    ...
    /// Read (and decode) the manifest for a given domain. Returns nil
    /// if the file is missing or unparseable. Used by the extension.
    static func loadManifest(domainSubdir: String) -> ClipboardManifest? {
        guard let url = manifestURL(domainSubdir: domainSubdir),
              let data = try? Data(contentsOf: url) else { return nil }
        return try? JSONDecoder().decode(ClipboardManifest.self, from: data)
    }
